...
def extract_conv_feats(vgg_model_path = './imagenet-vgg-verydeep-19.mat', batch_size = 25, noise_type='sap_noise_img_0.1'):
    """
    Args:
    ...
    with tf.Session() as sess:
        ...
        for split in ['test']:
            feat_save_path = './Flickr8k/conv_features_'+noise_type+'/%s_features.hkl' % split
            if not os.path.exists('./Flickr8k/conv_features_'+noise_type):
                os.makedirs('./Flickr8k/conv_features_'+noise_type)
                
            img_path = './Flickr8k/'+noise_type+'/'+ split
            img_fnames = os.listdir(img_path)
            img_path = ['./Flickr8k/'+noise_type+'/'+split+'/'+img_fname for img_fname in img_fnames]
...
def extract_caps(lower= True, noise_type='sap_noise_img_0.1'):
    """
    Args:
    ...
    for split in ['train', 'dev', 'test']:
        ...
        cap_save_path = './Flickr8k/cap_features_'+noise_type+'/%s.pkl' % split   
        if not os.path.exists('./Flickr8k/cap_features_'+noise_type):
                    os.makedirs('./Flickr8k/cap_features_'+noise_type)
    ...
    with open('./Flickr8k/cap_features_'+noise_type+'/dictionary.pkl', 'wb') as f:
...
if __name__ == "__main__":
    ...
    (1) 'sap_noise_img_0.1'
    (2) 'block_noise_img_4'
    (3) 'object_noise_img_cat'
    """
    extract_caps(lower =  True, noise_type='sap_noise_img_0.025')
    extract_conv_feats(vgg_model_path = './imagenet-vgg-verydeep-19.mat', batch_size = 25, noise_type='sap_noise_img_0.025')