...
def param_init_lstm(options, params, prefix='lstm', nin=None, dim=None):
    ...
    """
    # input weights (input gate, forget gate, memory cell, output gate)
...
def lstm_layer(tparams, state_below, options, prefix='lstm', mask=None, **kwargs):
    ...
    def _step(m_, x_, h_, c_):
        ...
        c = tensor.tanh(_slice(preact, 3, dim)) # memory cell

        c = f * c_ + i * c # memory cell
        h = o * tensor.tanh(c) # hidden state

        return h, c, i, f, o, preact

    state_below = tensor.dot(state_below, tparams[_p(prefix, 'W')]) + tparams[_p(prefix, 'b')]
    
    # step-by-step loop/update the LSTM layer for nsteps
...
def lstm_cond_layer(tparams, state_below, options, prefix='lstm',
    ...
    def _step(m_, x_, h_, c_, a_, as_, ct_, pctx_, dp_=None, dp_att_=None):
        ...
        # hidden state (h_) + previous word (x_) -> activations
        preact = tensor.dot(h_, tparams[_p(prefix, 'U')])
        preact += x_
...
def gen_sample(tparams, f_init, f_next, ctx0, options,
               trng=None, k=15, maxlen=100, stochastic=False):
    """Generate captions with beam search.
    
    This function uses the beamgen_sample search algorithm to conditionally
    ...
    for idx, ctx in enumerate(split):
            ...
            for tok in best_sample:
                ...
                if tok in word_idict:
                    ...
                    sample_output += 'UNK' + " "
            sample_output = sample_output.rstrip() # strip the right space
            handle.write("%s\n" % sample_output)
    handle.close()
    print "done"
...
def train(dim_word=100,  # word vector dimensionality
    ...
    if valid:
        ...
        kf_valid = KFold(n_splits=int(len(valid[0])/valid_batch_size), shuffle=False).split(valid[0])
    if test:
        #kf_test = KFold(len(test[0]), n_folds=len(test[0])/valid_batch_size, shuffle=False)
        kf_test = KFold(n_splits=int(len(test[0])/valid_batch_size), shuffle=False).split(test[0])
    ...
    # reload history
    if use_metrics==False and reload_ and os.path.exists(saveto):
        history_errs = numpy.load(saveto)['history_errs'].tolist()
    elif use_metrics==True and reload_ and os.path.exists(saveto):
        history_metrics = numpy.load(saveto)['history_metrics'].tolist()
    ...
    uidx = 0
    estop = False
    best_err = numpy.inf # best negative log likelihood
    best_metric = 0 # best evaluation metric
    for eidx in xrange(max_epochs):
        ...
        for caps in train_iter:
            ...
            if numpy.mod(uidx, dispFreq) == 0:
                ...
                print 'Epoch %d Update %d Cost %.4f PD %.2f UD %.2f' % (eidx, uidx, cost, pd_duration, ud_duration)
            ...
            if numpy.mod(uidx, saveFreq) == 0:
                ...
                if use_metrics==False:
                    ...
                    numpy.savez(saveto, history_metrics=history_metrics, **params)
            ...
            if numpy.mod(uidx, sampleFreq) == 0:
                ...
                for jj in xrange(numpy.minimum(10, len(caps))):
                    samples, scores = gen_sample(tparams, f_init, f_next, ctx_s[jj], model_options,
                                               trng=trng, k=15, maxlen=100, stochastic=False)
                    # adjust for length bias
                    lengths = numpy.array([len(s) for s in samples])
                    scores = scores / lengths
                    best_sample = samples[numpy.argmin(scores)]
            ...
            if numpy.mod(uidx, validFreq) == 0:
                ...
                print 'Train ', cost, 'Valid ', valid_err, 'Test ', test_err
            
                if use_metrics and eidx > 0: # if you use metric to validate
                    # Don't waste time generating descriptions during the first epoch of training
                        ...
                        m_score = check_metrics(generate_to+'-dev',
                                                dev_references)
                        m_score = m_score[metric]
                        print "%s %.4f" % (metric, m_score)   
                        history_metrics.append(m_score)
                        ...
                        if eidx > patience and len(history_metrics) > patience and m_score <= numpy.array(history_metrics)[:-patience].max():
                            bad_counter += 1
                ...
                else: # if you use cost to validate
        ...
        if save_per_epoch==True and use_metrics==False:
            numpy.savez(saveto + '_epoch_' + str(eidx + 1), history_errs=history_errs, **unzip(tparams))
        elif save_per_epoch==True and use_metrics==True:
            numpy.savez(saveto + '_epoch_' + str(eidx + 1), history_metrics=history_metrics, **unzip(tparams))
    ...
    generate_outputs(valid[1], generate_to+'-dev',
                     ...
                     f_next, model_options, trng,
                     word_idict)
    generate_outputs(test[1], generate_to+'-test',
                     use_noise, tparams, f_init,
                     f_next, model_options, trng,
                     word_idict)
    m_scores_dev = check_metrics(generate_to+'-dev',
                            dev_references)
    m_scores_test = check_metrics(generate_to+'-test',
                            test_references)
    print 'dev metric socres:', m_scores_dev, 'test metric scores:', m_scores_test      
               
    params = copy.copy(best_p)
    if use_metrics==False:
        numpy.savez(saveto, zipped_params=best_p, train_err=train_err, valid_err=valid_err, test_err=test_err, history_errs=history_errs, **params)
        return train_err, valid_err, test_err
    else:
        numpy.savez(saveto, zipped_params=best_p, m_score=m_scores_dev[metric], history_metrics=history_metrics,**params)
        return m_scores_dev[metric]
